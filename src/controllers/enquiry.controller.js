...
async function listEnquiries(req, res) {
  const rows = await enquiryService.listEnquiries({
    fromDate: req.query.fromDate,
    toDate: req.query.toDate,
    leadStatus: req.query.leadStatus,
    demoStatus: req.query.demoStatus,
  });
  return ok(res, rows, 'Enquiries fetched');
}

async function updateEnquiry(req, res) {
  const row = await enquiryService.updateEnquiry(req.params.id, req.validated.body);
